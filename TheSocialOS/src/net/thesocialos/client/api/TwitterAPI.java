...
public class TwitterAPI {
	...
	public void loadHomeTimelineInPanel(final HasWidgets panel) {
		 ...
		 * @Override public void onResponseReceived(Request request, Response response) { JSONArray array =
		 * JSONParser.parseStrict(response.getText()).isArray(); Window.alert(array.size()+ ""); for (int i = 0; i <
		 * array.size(); i++) { Tweet tweet = new Tweet(); tweet.id =
		 * array.get(i).isObject().get("id").isNumber().toString(); Window.alert(tweet.id); JSONObject user =
		 * array.get(i).isObject().get("user").isObject(); tweet.user_id = user.get("id").isNumber().toString();
		 * Window.alert(tweet.user_id); tweet.screen_name = user.get("screen_name").isString().toString();
		 * Window.alert(tweet.screen_name); tweet.profile_image_url =
		 * user.get("profile_image_url").isString().toString(); Window.alert(tweet.profile_image_url);
		 * tweet.user_verified = user.get("verified").isBoolean().booleanValue(); Window.alert(tweet.user_verified+"");
		 * tweet.text = array.get(i).isObject().get("text").isString().stringValue(); Window.alert(tweet.text);
		 * tweet.source = array.get(i).isObject().get("source").isString().stringValue(); Window.alert(tweet.source);
		 * panel.add(new TimelinePost(tweet)); } }
		 * @Override public void onError(Request request, Throwable exception) { // TODO Auto-generated method stub }
		 * });
		 */
	}
