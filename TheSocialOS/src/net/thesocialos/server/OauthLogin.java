...
public class OauthLogin extends HttpServlet {
	...
	public void service(HttpServletRequest request, HttpServletResponse response) {
		...
		if (serviceType.equalsIgnoreCase("twitter")) {
			...
			.apiKey("KQ6iX3bbpfDDpKGYtx2X8A")
			.apiSecret("VKwB7Ro0oCvePk1YnqlXLK9b1hdliEDF0qClr9U8w")
			.callback("http://www.thesocialos.net/twittercallback")
			.build();
		} else if (serviceType.equalsIgnoreCase("flickr")) {
			service = new ServiceBuilder()
			.provider(FlickrApi.class)
			.apiKey("cc01bd2671d139d49a97d10179ff6341")
			.apiSecret("3d016edebfd20a11")
			.callback("http://www.thesocialos.net/flickrcallback")
			.build();
		} else {
			PrintWriter writer;
			try {
