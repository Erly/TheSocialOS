...
public class ServiceSecurityLayer extends RemoteServiceServlet{
	  ...
	  public String processCall(String payload) throws SerializationException {
	    ...
	    try {
	      ...
	      if (checkSession(rpcRequest)==null){ //Check the intrusion
	    	  return RPC.encodeResponseForFailure(null, new IncompatibleRemoteServiceException("La excepción de los huevos",new Throwable("Warning full UP")));
	      }
		...
		try{
			...
			if (session != null){ 
				...
				if (user == null || !session.getSessionID().equals(sid)) {//Verifing session id
					Log.debug("SecurityLayer-->session altered");
					return null;
				}else{
					return user;
				}
	...
	private Boolean checkSession(RPCRequest rpcRequest){
		...
		if (rpcRequest.getMethod().getName().equals("login") || rpcRequest.getMethod().getName().equals("logout") ||
				rpcRequest.getMethod().getName().equals("register") /*|| rpcRequest.getMethod().getName().equals("getLoggedUser")*/){
			return true;
		}else{
			 if(checkSession()!=null){return true;}else{return false;}
